...
- schema_version
- correlation_id
- idempotency_key (workflow) and step_dedupe_key (when applicable)
...
- Elevated quarantine rate:
  - quarantined / started above threshold (especially provider-specific)
- Provider health:
...
- `job_run_id` is the operational join key across:
  ...
  - published events
  - logs
- `correlation_id` joins cross-workflow chains (e.g., IngestNewNova -> downstream workflows).
